...
import { CORE_MEMORY_FIELDS, containsSensitiveText, validateMemoryRecord } from "../memory-schema.js";
...
export async function readObsidianSource(source, projectRoot) {
  ...
  for (const filePath of files) {
    ...
    const requiredFields = unique([...CORE_MEMORY_FIELDS, ...(source.required_frontmatter ?? [])]);
    const documentIssues = validateMemoryRecord(attributes, { requiredFields });
    if (containsSensitiveText(body)) documentIssues.push("body contains a sensitive marker");
    if (documentIssues.length > 0) {
      issues.push(...documentIssues.map((issue) => `${source.id}:${relativePath} frontmatter ${issue}`));
      continue;
    }
